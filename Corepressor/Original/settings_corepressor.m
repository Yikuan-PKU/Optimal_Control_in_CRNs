function options = settings_corepressor(varargin)
...
%---------------------------------------
% NOTE: Support for Direct Multiple Shooting has been removed in version 2.5. Users for Direct Multiple Shooting are adviced to use ICLOCS version 1 or 2, or other better choices e.g. ACADOS
...
options.perturbation.H=[0.5*(eps/2)^(1/3)];  % Perturbation size for the second derivatives
options.perturbation.J=[(8*eps)^(1/3)];  % Perturbation size for the first derivatives
...
options.ipopt.max_iter=10000;                   % Maximum number of iterations. The default value is 3000.
...
options.ipopt.limited_memory_max_history=10;   % Maximum size of the history for the limited quasi-Newton Hessian approximation. The valid range for this integer option is [0, +inf) 
                                               % and its default value is 6. 
options.ipopt.limited_memory_max_skipping=2;  % Threshold for successive iterations where update is skipped for the quasi-Newton approximation.
                                               % The valid range for this integer option is [1,+inf) and its default value is 2. 

% fmincon settings (NOT RECOMMENDED!)
%---------------------------------------
% See website for detailed info on how to enable the use of fmincon
...
options.meshstrategy='hp_flexible';
