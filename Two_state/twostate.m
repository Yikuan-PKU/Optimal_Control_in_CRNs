...
% State error bounds
problem.states.xErrorTol_local=0.001; 
problem.states.xErrorTol_integral=0.01; 

% State constraint error bounds
problem.states.xConstraintTol=0.001;
...
problem.inputs.uu=[K K];

% Bounds on the first control action
problem.inputs.u0l=[0 0];
problem.inputs.u0u=[K K];

% Input rate bounds
%problem.inputs.url=[]; 
%problem.inputs.uru=[];
% Input constraint error bounds
problem.inputs.uConstraintTol=[0.001 0.001];
...
problem.constraints.gTol_eq=[0.001]; % equality cosntraint error bounds
...
stageCost = (u(:,1).*x(:,1)-u(:,2).*(1-x(:,1))).*log((u(:,1).*x(:,1))./(u(:,2).*(1-x(:,1))));
%------------- END OF CODE --------------
...
boundaryCost=0;%TerminalCost(xf,[data.kf(1) data.kf(2)]);
