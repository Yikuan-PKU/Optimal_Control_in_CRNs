function [problem,guess] = twostate(tau,para)
...
InternalDynamics=@twostate_Dynamics_Internal; 
SimDynamics=@twostate_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_twostate;
...
problem.inputs.uu=[K1 K1 K2 K2 K3 K3];

% Bounds on the first control action
problem.inputs.u0l=[0 0 0 0 0 0];
problem.inputs.u0u=[K1 K1 K2 K2 K3 K3];

% Input rate bounds
%problem.inputs.url=[]; 
%problem.inputs.uru=[];
% Input constraint error bounds
problem.inputs.uConstraintTol=[0.01 0.01 0.01 0.01 0.01 0.01];
%problem.inputs.urConstraintTol=[0.1 0.1];

% Guess the input sequences with [u0 ... uf]
guess.inputs(:,1)=[ki(1) kf(1)];
guess.inputs(:,2)=[ki(2) kf(2)];
guess.inputs(:,3)=[ki(3) kf(3)];
guess.inputs(:,4)=[ki(4) kf(4)];
guess.inputs(:,5)=[ki(5) kf(5)];
guess.inputs(:,6)=[ki(6) kf(6)];
% Choose the set-points if required
problem.setpoints.states=[];
problem.setpoints.inputs=[];
% Path constraint function 
problem.constraints.ng_eq=3; % number of quality constraints in format of g(x,u,p,t) == 0
problem.constraints.gTol_eq=[0.01,0.01,0.01]; % equality cosntraint error bounds
...
problem.data.K1=para(1);
problem.data.K2=para(2);
problem.data.K3=para(3);
problem.data.ki=ki;
problem.data.kf=kf;
...
stageCost = (x(:,1).*u(:,1)-x(:,2).*u(:,2)).*log((x(:,1).*u(:,1))./(x(:,2).*u(:,2)))+(x(:,1).*u(:,3)-(1-x(:,1)-x(:,2)).*u(:,4)).*log((x(:,1).*u(:,3))./((1-x(:,1)-x(:,2)).*u(:,4)))+(x(:,2).*u(:,5)-(1-x(:,1)-x(:,2)).*u(:,6)).*log((x(:,2).*u(:,5))./((1-x(:,1)-x(:,2)).*u(:,6)));
